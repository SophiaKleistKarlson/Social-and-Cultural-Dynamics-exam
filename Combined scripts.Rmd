...
for (i in 1:39) {
  ...
  inc_ID <- incorrect_conf %>% filter(Subject_ID == i) 
  cor_ID <- correct_conf %>% filter(Subject_ID == i)
  
  Res <- mean(cor_ID$abs_conf_1) - mean(inc_ID$abs_conf_1)
  Resolution[i] <- paste(Res, collapse=NULL)
}
...
Calibration
Described by Karsten Olsen as following (in Danish):

"Vi gør følgende for hver participant:
-Vi har 5 confidence levels
-Vi udregner average choice accuracy for hvert confidence level - dvs mean(accuracy(index)) hvor index indikerer de trials deltageren brugte et givet confidence level
-Vi udregner forskellen mellem confidence level og average choice accuracy for hvert confidence level - fx .6-.52= .08, .7 - .72 = -.02 osv.
-Vi summerer disse forskelle på tværs af confidence levels"
...
for (i in 1:39){
  cal_0.6 <- cal_df %>% filter(Subject_ID == i & abs_conf_1 == 0.6) 
  cal_0.7 <- cal_df %>% filter(Subject_ID == i & abs_conf_1 == 0.7) 
  cal_0.8 <- cal_df %>% filter(Subject_ID == i & abs_conf_1 == 0.8) 
  cal_0.9 <- cal_df %>% filter(Subject_ID == i & abs_conf_1 == 0.9)
  cal_1.0 <- cal_df %>% filter(Subject_ID == i & abs_conf_1 == 1.0) 
  ...
  mean_acc <- c(mean_0.6, mean_0.7, mean_0.8, mean_0.9, mean_1.0)
  conf <- c(0.6, 0.7, 0.8, 0.9, 1.0)
  Calibrations <- sum(conf - mean_acc, na.rm = T)
  Calibration[i] <- paste(Calibrations, collapse=NULL)
}
...
pp_check(skep_sus_mod0, nsamples = 100) # 
...
pp_check(skep_sus_mod1, nsamples = 100)
...
df_meta$crit_Susceptibility <- sus_crit$sus_crit
write.csv(df_meta, "Meta-measures.csv")
```
